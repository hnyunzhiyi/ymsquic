    ...
    )
{
    ...
    if (Status == STATUS_PENDING) {
        ...
        QuicTraceLogWarning(
            "DatapathOpenTcpSocketFailed: [udp] RSS helper socket failed to open, 0x%x",
            Status);
        goto Error;
    }
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceLogWarning(
            "DatapathOpenTcpSocketFailedAsync: [udp] RSS helper socket failed to open (async), 0x%x",
            Status);
        goto Error;
    }
    ...
    if (Status == STATUS_PENDING) {
        ...
        QuicTraceLogWarning(
            "DatapathQueryRssScalabilityInfoFailed: [udp] Query for SIO_QUERY_RSS_SCALABILITY_INFO failed, 0x%x",
            Status);
        goto Error;
    }
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceLogWarning(
            "DatapathQueryRssScalabilityInfoFailedAsync: [udp] Query for SIO_QUERY_RSS_SCALABILITY_INFO failed (async), 0x%x",
            Status);
        goto Error;
    }
    ...
    )
{
    ...
    if (Status == STATUS_PENDING) {
        ...
        QuicTraceLogWarning(
            "DatapathOpenUdpSocketFailed: [udp] UDP send segmentation helper socket failed to open, 0x%x",
            Status);
        goto Error;
    }
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceLogWarning(
            "DatapathOpenUdpSocketFailedAsync£º[udp] UDP send segmentation helper socket failed to open (async), 0x%x",
            Status);
        goto Error;
    }
    ...
    do {
        ...
        if (Status == STATUS_PENDING) {
            ...
            QuicTraceLogWarning(
                "DatapathQueryUdpSendMsgFailed£º[udp] Query for UDP_SEND_MSG_SIZE failed, 0x%x",
                Status);
            break;
        }
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceLogWarning(
                "DatapathQueryUdpSendMsgFailedAsync: [udp] Query for UDP_SEND_MSG_SIZE failed (async), 0x%x",
                Status);
            break;
        }
    ...
    do {
        ...
        if (Status == STATUS_PENDING) {
            ...
            QuicTraceLogWarning(
                "DatapathQueryRecvMaxCoalescedSizeFailed: [udp] Query for UDP_RECV_MAX_COALESCED_SIZE failed, 0x%x",
                Status);
            break;
        }
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceLogWarning(
                "DatapathQueryRecvMaxCoalescedSizeFailedAsync: [udp] Query for UDP_RECV_MAX_COALESCED_SIZE failed (async), 0x%x",
                Status);
            break;
        }
    ...
    )
{
    ...
    while (DataIndicationHead != NULL) {
        ...
        if (DataIndication->Buffer.Mdl == NULL ||
            ...
            QuicTraceLogWarning(
                "DatapathDropEmptyMdl: [%p] Dropping datagram with empty mdl.",
                Binding);
            goto Drop;
        }
        ...
        if (!FoundLocalAddr && !IsUnreachableError) {
            ...
            //
            QuicTraceLogWarning(
                "DatapathDropMissingInfo: [%p] Dropping datagram missing IP_PKTINFO/IP_RECVERR.",
                Binding);
            goto Drop;
        }
...
#if QUIC_CLOG
            QuicTraceLogVerbose(
                "DatapathUnreachable: [sock][%p] Unreachable error from addr:%p, Length:%d",
                Binding,
        ...
        if (MessageLength == 0) {
            ...
            if (DataLength > MAXUINT16) {
                QuicTraceLogWarning(
                    "DatapathDropTooBig: [%p] Dropping datagram with too many bytes (%llu).",
                    Binding,
                    (uint64_t)DataLength);
                goto Drop;
            }
        ...
        if (!QuicMdlMapChain(DataIndication->Buffer.Mdl)) {
            QuicTraceLogWarning(
                "DatapathDropMdlMapFailure: [%p] Failed to map MDL chain",
                Binding);
            goto Drop;
        }

        QuicTraceLogVerbose(
            "DatapathRecv: [udp][%p] Recv %u bytes (segment=%hu) Src(addr:%p, Length:%d) Dst(addr:%p, Length:%d)",
            Binding,
            (uint32_t)DataLength,
            MessageLength,
        ...
        for ( ; DataLength != 0; DataLength -= MessageLength) {
            ...
            if ((SIZE_T)MessageLength > Mdl->ByteCount - MdlOffset) {
                QuicTraceLogWarning(
                    "DatapathFragmented: [%p] Dropping datagram with fragmented MDL.",
                    Binding);
            ...
            if (RecvContext == NULL) {
                ...
                if (RecvContext == NULL) {
                    QuicTraceLogWarning(
                        "DatapathDropAllocRecvContextFailure: [%p] Couldn't allocate receive context.",
                        Binding);
                    goto Drop;
                }
                ...
                if (RecvContext->ProcContext->OutstandingPendingBytes >= PENDING_BUFFER_LIMIT) {
                    ...
                    if (RecvContext->DataBufferStart == NULL) {
                        QuicTraceLogWarning(
                            "DatapathDropAllocRecvBufferFailure: [%p] Couldn't allocate receive buffers.",
                            Binding);
                        goto Drop;
                    }
            ...
            *DatagramChainTail = Datagram;
            ...
            if (++RecvContext->ReferenceCount == URO_MAX_DATAGRAMS_PER_INDICATION) {
                QuicTraceLogWarning(
                    "DatapathUroExceeded: [%p] Exceeded URO preallocation capacity.",
                    Binding);
                break;
            }
    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "DatapathSendTo: [udp][%p] Send %u bytes in %hhu buffers (segment=%hu) Dst(addr:%p, Length:%d)",
        Binding,
        SendContext->TotalSize,
        SendContext->WskBufferCount,
        SendContext->SegmentSize,
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceLogWarning(
            "DatapathErrorStatus£º[udp][%p] ERROR, %u, %s.",
            Binding,
            Status,
            "WskSendMessages");
        //
    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "DatapathSendFromTo: [udp][%p] Send %u bytes in %hhu buffers (segment=%hu) Dst(addr:%p, Length:%d), Src(addr:%p, Length:%d)",
        Binding,
        SendContext->TotalSize,
        SendContext->WskBufferCount,
        SendContext->SegmentSize,
