    ...
    )
{
    ...
    QuicTraceLogInfo("WindowsUserLoaded: [dll] Loaded");
}
...
{
    QuicTraceLogInfo("WindowsUserUnloaded: [dll] Unloaded");
    ...
    )
{
    ...
    QuicTraceLogInfo(
        "WindowsUserProcessorState: [dll] Processors:%u, Groups:%u, NUMA Nodes:%u",
        ...
        while (Offset < BufferLength) {
            ...
            if (Info->Relationship == RelationNumaNode) {
                if (Info->NumaNode.GroupMask.Group == QuicProcessorInfo[Index].Group &&
                    ...
                    QuicTraceLogInfo(
                        "ProcessorInfo: [ dll] Proc[%u] Group[%hu] Index[%u] NUMA[%u]",
                        Index,
    ...
    )
{
    ...
    QuicTraceLogError(
        "WindowsUserInitialized: [dll] Initialized (AvailMem = %llu bytes)",
        QuicTotalMemory);
    ...
    )
{
    ...
    HeapDestroy(QuicPlatform.Heap);
    QuicPlatform.Heap = NULL;
    QuicTraceLogInfo("WindowsUserUninitialized: [dll] Uninitialized");
}
