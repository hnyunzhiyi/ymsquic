...
CleanTestCertificatesFromStore(BOOLEAN UserStore)
{
    ...
    QuicTraceLogInfo(
        "CertCleanTestCerts: [cert] %d test certificates found, and %d deleted",
        Found,
        Deleted);
    ...
    )
{
    ...
    if (FAILED(hr)) {
        QuicTraceLogError(
            "LibraryErrorStatus£º[lib] ERROR, %u, %s.",
            hr,
...
ReadKey:
    ...
    if (hr == ERROR_SUCCESS) {
        QuicTraceLogInfo(
            "CertOpenRsaKeySuccess: [cert] Successfully opened RSA key");
        goto Cleanup;
    ...
    QuicTraceLogInfo("CertCreateRsaKeySuccess: [cert] Successfully created key");
    ...
    )
{
    ...
    if (GetLastError() == ERROR_ALREADY_EXISTS) {
        QuicTraceLogInfo(
            "CertCreationEventAlreadyCreated: [test] CreateEvent opened existing event");
        DWORD WaitResult = WaitForSingleObject(Event, QUIC_CERT_CREATION_EVENT_WAIT);
        if (WaitResult != WAIT_OBJECT_0) {
            QuicTraceLogWarning(
                "CertWaitForCreationEvent: [test] WaitForSingleObject returned 0x%x, proceeding without caution... (GLE: 0x%x)",
                WaitResult,
                GetLastError());
        }
    } else {
