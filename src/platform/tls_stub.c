    ...
    )
{
    ...
    QuicTraceLogConnVerbose("StubTlsContextCreated: TLS context Created");
    ...
    if (TlsContext->ResumptionTicketBuffer != NULL) {
        QuicTraceLogConnVerbose("StubTlsUsing0Rtt: Using 0-RTT ticket.");
    }
...
{
    if (TlsContext != NULL) {
        QuicTraceLogConnVerbose("StubTlsContextCleaningUp: Cleaning up");
...
{
    QuicTraceLogConnInfo("StubTlsContextReset: Resetting TLS state");
    ...
    case QUIC_TLS_MESSAGE_SERVER_HANDSHAKE: {
        if (ClientMessage->Type == QUIC_TLS_MESSAGE_CLIENT_HANDSHAKE) {
            ...
            *ResultFlags |= QUIC_TLS_RESULT_COMPLETE;
            ...
            QuicTraceLogConnInfo("StubTlsHandshakeComplete: Handshake complete");
    ...
    )
{
        ...
        if (ServerMessage->Type == QUIC_TLS_MESSAGE_SERVER_INITIAL) {
            ...
            *ResultFlags |= QUIC_TLS_RESULT_WRITE_KEY_UPDATED;
            ...
            if (TlsContext->SecConfig->Flags & QUIC_CREDENTIAL_FLAG_NO_CERTIFICATE_VALIDATION) {
                QuicTraceLogConnWarning("StubTlsCertValidationDisabled: Certificate validation disabled!");
            } else {
    ...
    case QUIC_TLS_MESSAGE_CLIENT_HANDSHAKE: {
        ...
        QuicTraceLogConnVerbose(
            "StubTlsRecvNewSessionTicket: Received new ticket. ticket_len:%u for %s",
            ServerMessageLength,
            TlsContext->SNI);
    ...
    )
{
    if (*BufferLength) {
        QuicTraceLogConnVerbose(
            "StubTlsProcessData: Processing %u received bytes",
            *BufferLength);
    }
    ...
    if (QuicTlsHasValidMessageToProcess(TlsContext, *BufferLength, Buffer)) {
        ...
        QuicTraceLogConnInfo(
            "StubTlsConsumedData: Consumed %u bytes",
            *BufferLength);

        if (State->BufferLength > PrevBufferLength) {
            QuicTraceLogConnInfo(
                "StubTlsProducedData: Produced %hu bytes",
