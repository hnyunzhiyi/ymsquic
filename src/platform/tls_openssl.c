    ...
    )
{
    ...
    QuicTraceLogConnVerbose(
        "OpenSslNewEncryptionSecrets: New encryption secrets (Level = %u)",
        Level);
    ...
    )
{
    ...
    QuicTraceLogConnVerbose(
        "OpenSslAddHandshakeData: Sending %llu handshake bytes (Level = %u)",
        Length,
        Level);
    ...
    case QUIC_PACKET_KEY_HANDSHAKE:
        if (TlsState->BufferOffsetHandshake == 0) {
            ...
            QuicTraceLogConnInfo(
                "OpenSslHandshakeDataStart: Writing Handshake data starts at %u",
    ...
    case QUIC_PACKET_KEY_1_RTT:
        if (TlsState->BufferOffset1Rtt == 0) {
            ...
            QuicTraceLogConnInfo(
                "OpenSsl1RttDataStart: Writing 1-RTT data starts at %u",
    ...
    )
{
    ...
    QuicTraceLogConnError(
        "OpenSslAlert: Send alert = %u (Level = %u)",
        Alert,
        Level);
    ...
    )
{
    ...
    QuicTraceLogConnVerbose("OpenSslContextCreated: TLS context Created");
...
{
    QuicTraceLogConnInfo("OpenSslContextReset: Resetting TLS state");
    ...
    )
{
    ...
    if (DataType == QUIC_TLS_TICKET_DATA) {
        ...
        QuicTraceLogConnVerbose(
            "OpenSsslIgnoringTicket: Ignoring %u ticket bytes",
            *BufferLength);
        goto Exit;
    }
    ...
    if (*BufferLength != 0) {
        QuicTraceLogConnVerbose(
            "OpenSslProcessData: Processing %u received bytes",
            *BufferLength);
    }
    ...
    if (!State->HandshakeComplete) {
        ...
        if (Ret <= 0) {
            ...
            case SSL_ERROR_SSL:
                QuicTraceLogConnError(
                    "OpenSslHandshakeErrorStr: TLS handshake error: %s",
            ...
            default:
                QuicTraceLogConnError(
                    "OpenSslHandshakeError: TLS handshake error: %d",
                    Err);
                TlsContext->ResultFlags |= QUIC_TLS_RESULT_ERROR;
                goto Exit;
            }
        ...
        if (!TlsContext->IsServer) {
            ...
            if (NegotiatedAlpnLength == 0) {
                QuicTraceLogConnError("OpenSslAlpnNegotiationFailure: Failed to negotiate ALPN");
                TlsContext->ResultFlags |= QUIC_TLS_RESULT_ERROR;
                goto Exit;
            }
            if (NegotiatedAlpnLength > UINT8_MAX) {
                QuicTraceLogConnError("OpenSslInvalidAlpnLength: Invalid negotiated ALPN length");
                TlsContext->ResultFlags |= QUIC_TLS_RESULT_ERROR;
                goto Exit;
            }
            ...
            if (TlsContext->State->NegotiatedAlpn == NULL) {
                QuicTraceLogConnError("OpenSslNoMatchingAlpn: Failed to find a matching ALPN");
                TlsContext->ResultFlags |= QUIC_TLS_RESULT_ERROR;
                goto Exit;
            }
        }

        QuicTraceLogConnInfo("OpenSslHandshakeComplete: Handshake complete");
        ...
        if (TlsContext->IsServer) {
            ...
            if (TransportParams == NULL || TransportParamLen == 0) {
                QuicTraceLogConnError("OpenSslMissingTransportParameters: No transport parameters received");
                TlsContext->ResultFlags |= QUIC_TLS_RESULT_ERROR;
                goto Exit;
            }
            if (!TlsContext->ReceiveTPCallback(
    ...
    if (Ret != 1) {
        ...
        case SSL_ERROR_SSL:
            QuicTraceLogConnError("OpenSslHandshakeErrorStr: TLS handshake error: %s",
                ERR_error_string(ERR_get_error(), NULL));
            TlsContext->ResultFlags |= QUIC_TLS_RESULT_ERROR;
            goto Exit;

        default:
            QuicTraceLogConnError("OpenSslHandshakeError: TLS handshake error: %d", Err);
            TlsContext->ResultFlags |= QUIC_TLS_RESULT_ERROR;
            goto Exit;
        }
    }
