    ...
    )
{
    ...
    QuicTraceLogVerbose("MiTlsInitialize: [tls] Initializing miTLS library");
...
{
    QuicTraceLogVerbose("MiTlsUninitialize: [tls] Cleaning up miTLS library");
    FFI_mitls_cleanup();
    TlsFree(miTlsCurrentConnectionIndex);
}
    ...
    if (Config->IsServer) {
        ...
        if (Config->ServerName != NULL) {
            ...
            if (Config->ResumptionTicketBuffer != NULL) {
                ...
                QuicTraceLogConnVerbose("MiTlsUsing0Rtt: Using 0-RTT ticket.");
    ...
    )
{
    ...
    if (DataType == QUIC_TLS_CRYPTO_DATA) {
        ...
        if (*BufferLength) {
            QuicTraceLogConnVerbose("MiTlsProcess: Processing %u bytes", *BufferLength);
        ...
        QuicTraceLogConnVerbose("MiTlsSend0RttTicket: Sending 0-RTT ticket");
    ...
    )
{
    ...
    while (!(ResultFlags & QUIC_TLS_RESULT_ERROR)) {
        ...
        QuicTraceLogConnVerbose("MiTlsFfiProces: FFI_mitls_quic_process processing %u input bytes", (uint32_t)Context.input_len);
        ...
        if (!FFI_mitls_quic_process(TlsContext->miTlsState, &Context)) {
            QuicTraceLogConnError(
                "MiTlsFfiProcessFailed: FFI_mitls_quic_process failed, tls_error %hu, %s",
                Context.tls_error,
        ...
        QuicTraceLogConnVerbose(
            "MiTlsFfiProcessResult: FFI_mitls_quic_process read %u bytes and has %u bytes ready to send",
        ...
        if (Context.flags & QFLAG_COMPLETE && !State->HandshakeComplete) {
            QuicTraceLogConnVerbose("MiTlsHandshakeComplete: Handshake complete");
        ...
        if (Context.flags & QFLAG_REJECTED_0RTT) {
            ...
            QuicTraceLogConnVerbose("MiTlsEarlyDataRejected: Early data rejected");
        }
        ...
        if (!TlsContext->TlsKeyScheduleSet) {
            ...
            if (TlsContext->IsServer) {
                if (ReadKeyUpdated) {
                    ...
                    QuicTraceLogConnVerbose("MiTlsEarlyDataAccepted: Early data accepted");
                    ...
                    if (!FFI_mitls_get_hello_summary(
                            ...
                            &Ticket, &TicketLen)) {
                        QuicTraceLogConnError(
                            "MiTlsFfiGetHelloSummaryFailed: FFI_mitls_get_hello_summary failed, cookie_len: %zu, ticket_len: %zu",
                            CookieLen,
                            TicketLen);
                        ResultFlags |= QUIC_TLS_RESULT_ERROR;
                        break;
                    ...
                    if (!(Context.flags & QFLAG_REJECTED_0RTT)) {
                        QuicTraceLogConnVerbose("MiTlsEarlyDataNotAttempted: Early data not attempted");
                    }
                }
            } else {
                if (WriteKeyUpdated) {
                    QuicTraceLogConnVerbose("MiTlsEarlyDataAttempted: Early data attempted");
                    TlsContext->TlsKeySchedule = 1; // 0-RTT allowed.
                    TlsContext->EarlyDataAttempted = TRUE;
                } else {
                    TlsContext->TlsKeySchedule = 0;
            ...
            QuicTraceLogConnVerbose("MiTlsKeySchedule: Key schedule = %hu",
        ...
        if (ReadKeyUpdated) {
            ...
            if (QuicPacketKeyCreate(
                ...
                case QUIC_PACKET_KEY_0_RTT:
                    QuicTraceLogConnVerbose("MiTls0RttReadKeyExported: 0-RTT read key exported");
                    break;
                case QUIC_PACKET_KEY_HANDSHAKE:
                    QuicTraceLogConnVerbose("MiTlsHandshakeReadKeyExported: Handshake read key exported");
                    break;
                case QUIC_PACKET_KEY_1_RTT:
                    QuicTraceLogConnVerbose("MiTls1RttReadKeyExported: 1-RTT read key exported");
                    if (!TlsContext->IsServer) {
                        if (TlsContext->EarlyDataAttempted &&
        ...
        if (WriteKeyUpdated) {
            ...
            if (QuicPacketKeyCreate(
                ...
                switch (NewKeyType) {
                case QUIC_PACKET_KEY_0_RTT:
                    QuicTraceLogConnVerbose("MiTls0RttWriteKeyExported: 0-RTT write key exported");
                    break;
                case QUIC_PACKET_KEY_HANDSHAKE:
                    QuicTraceLogConnVerbose("MiTlsHandshakeWriteKeyExported: Handshake write key exported");
                    break;
                case QUIC_PACKET_KEY_1_RTT:
                    QuicTraceLogConnVerbose("MiTls1RttWriteKeyExported: 1-RTT write key exported");
                    break;
                default:
                    QUIC_FRE_ASSERT(FALSE);
                    break;
            ...
            case QUIC_PACKET_KEY_HANDSHAKE:
                ...
                QuicTraceLogConnVerbose("MiTlsHandshakeWriteOffsetSet: Handshake write offset = %u",
                    State->BufferOffsetHandshake);
                break;
            case QUIC_PACKET_KEY_1_RTT:
                State->BufferOffset1Rtt = State->BufferTotalLength;
                QuicTraceLogConnVerbose("MiTls1RttWriteOffsetSet: 1-RTT write offset = %u",
    ...
    *BufferConsumed = BufferOffset;
    ...
    QuicTraceLogConnVerbose("MiTlsFfiProcesComplete: Consumed %u bytes", BufferOffset);
    ...
    )
{
    ...
    QuicTraceLogConnVerbose("MiTlsOnCertSelect: OnCertSelect");

    //
    // Only allow TLS 1.3.
    //
    if (TlsVersion != TLS_1p3) {
        QuicTraceLogError("TlsError: [tls][%p] ERROR, %s.",
            TlsContext->Connection,
    ...
    QuicTraceLogConnInfo(
        "miTlsCertSelected: Server certificate selected. SNI: %s; Algorithm: 0x%4.4x",
        TlsContext->SNI,
        *SelectedSignature);
    ...
    )
{
    ...
    QuicTraceLogConnVerbose("MiTlsOnNegotiate: OnNegotiate");
    ...
    if (!TlsContext->IsServer) {
        ...
        QuicTraceLogConnVerbose(
            "miTlsProcessServerAlpn: Processing server ALPN (Length=%u)",
    ...
    )
{
    ...
    QuicTraceLogConnVerbose("MiTlsOnCertFormat: OnCertFormat");
    ...
    )
{
    ...
    QuicTraceLogConnVerbose("MiTlsOnCertSign: OnCertSign");
    ...
    )
{
    ...
    QuicTraceLogConnVerbose("MiTlsOnCertVerify: OnCertVerify");

    int Result = 0;
    QUIC_CERTIFICATE* Certificate = NULL;

    if (TlsContext->SecConfig->Flags & QUIC_CREDENTIAL_FLAG_NO_CERTIFICATE_VALIDATION) {
        QuicTraceLogConnWarning("MiTlsCertValidationDisabled: Certificate validation disabled!");
        Result = 1;
        goto Error;
    }
    ...
    )
{
    ...
    QuicTraceLogConnVerbose(
        "MiTlsRecvNewSessionTicket: Received new ticket. ticket_len:%u session_len:%u for %s",
