    ...
    )
{
    ...
    QuicTraceLogVerbose("SchannelInitialized: [tls] Library initialized");
    ...
    )
{
    ...
    QUIC_AES_GCM_ALG_HANDLE = NULL;
#endif
    QuicTraceLogVerbose("SchannelUninitialized: [tls] Library uninitialized");
}
    ...
    )
{
    if (CallbackData == NULL) {
        QuicTraceLogError("LibraryError: [lib] ERROR, %s.",
    ...
    )
{
    ...
    QuicTraceLogVerbose("SchannelAchAsync: [tls] Calling SspiAcquireCredentialsHandleAsyncW");
    ...
    if (SecStatus != SEC_E_OK) {
        QuicTraceLogVerbose("LibraryErrorStatus: [lib] ERROR, %u, %s.",
            SecStatus,
            "SspiAcquireCredentialsHandleAsyncW");
...
#pragma warning(pop)
        if (!NT_SUCCESS(Status)) {
            QuicTraceLogError("LibraryErrorStatus: [lib] ERROR, %u, %s.",
                Status,
    ...
    QuicTraceLogVerbose("SchannelAchWorkerStart: [tls] Starting ACH worker");
    ...
    QuicTraceLogVerbose("SchannelAch: [tls] Calling AcquireCredentialsHandleW");
    ...
    QuicTraceLogVerbose(
        "SchannelAchCompleteInline: [tls] Invoking security config completion callback inline, 0x%x",
        Status);
    ...
    )
{
    ...
    if (TlsContext == NULL) {
        QuicTraceLogError("AllocFailure: Allocation of '%s' failed. (%llu bytes)",
            "QUIC_TLS",
            sizeof(QUIC_TLS));
        Status = QUIC_STATUS_OUT_OF_MEMORY;
        goto Error;
    ...
    QuicTraceLogConnVerbose("TlsContext: TLS context Created");
...
{
    QuicTraceLogConnInfo("SchannelContextReset: Resetting TLS state");
    ...
    )
{
    ...
    if (*InBufferLength == 0) {
        ...
        if (TlsContext->SNI != NULL) {
            ...
            if (QUIC_FAILED(Status)) {
                QuicTraceLogError(
                    "TlsErrorStatus: [tls][%p] ERROR, %u, %s.",
    ...
    for (uint32_t i = 0; i < OutSecBufferDesc.cBuffers; ++i) {
        if (OutputTokenBuffer == NULL &&
            ...
            QuicTraceLogConnVerbose(
                "SchannelKeyReady: Key Ready Type, %u [%hu to %hu]",
    ...
    case SEC_E_OK:
        ...
        if (!State->HandshakeComplete) {
            ...
            QuicTraceLogConnInfo(
                "SchannelHandshakeComplete: Handshake complete (resume=%hu)",
    ...
    case SEC_I_CONTINUE_NEEDED_MESSAGE_OK:
        ...
        QuicTraceLogConnInfo(
            "SchannelConsumedBytes: Consumed %u bytes",
            *InBufferLength);
        ...
        for (uint8_t i = 0; i < NewPeerTrafficSecretsCount; ++i) {
            ...
            if (NewPeerTrafficSecrets[i]->TrafficSecretType == SecTrafficSecret_ClientEarlyData) {
                ...
                if (State->ReadKey == QUIC_PACKET_KEY_INITIAL) {
                    ...
                    QuicTraceLogConnInfo("SchannelReadHandshakeStart: Reading Handshake data starts now");
                    ...
                    QuicTraceLogConnInfo("SchannelRead1RttStart: Reading 1-RTT data starts now");
                }
            }
        }
        ...
        for (uint8_t i = 0; i < NewOwnTrafficSecretsCount; ++i) {
            ...
            if (NewOwnTrafficSecrets[i]->TrafficSecretType == SecTrafficSecret_ClientEarlyData) {
                ...
                if (State->WriteKey == QUIC_PACKET_KEY_INITIAL) {
                    ...
                    QuicTraceLogConnInfo(
                        "SchannelWriteHandshakeStart: Writing Handshake data starts at %u",
                        State->BufferOffsetHandshake);
                    ...
                    if (!TlsContext->IsServer && State->BufferOffsetHandshake == State->BufferOffset1Rtt) {
                        ...
                        QuicTraceLogConnInfo(
                            "SchannelWrite1RttStart: Writing 1-RTT data starts at %u",
        ...
        if (OutputTokenBuffer != NULL && OutputTokenBuffer->cbBuffer > 0) {
            ...
            QuicTraceLogConnInfo(
                "SchannelProducedData: Produced %u bytes",
    ...
    case SEC_E_INCOMPLETE_MESSAGE:
        ...
        *InBufferLength = 0;
        ...
        if (MissingBuffer != NULL && MissingBuffer->cbBuffer != 0) {
            QuicTraceLogConnInfo(
                "SchannelMissingData: TLS message missing %u bytes of data",
    ...
    )
{
    ...
    if (DataType == QUIC_TLS_TICKET_DATA) {
        ...
        QuicTraceLogConnVerbose(
            "SchannelIgnoringTicket: Ignoring %u ticket bytes",
            *BufferLength);
        goto Error;
    }
    ...
    QuicTraceLogConnVerbose(
        "SchannelProcessingData: Processing %u received bytes",
        *BufferLength);
