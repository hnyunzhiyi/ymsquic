    ...
    )
{
    ...
    if (Result != NO_ERROR) {
        int WsaError = WSAGetLastError();
        QuicTraceLogWarning(
            "DatapathQueryRssProcessorInfoFailed: [udp] Query for SIO_QUERY_RSS_SCALABILITY_INFO failed, 0x%x",
            WsaError);
        goto Error;
    }
...
#ifdef UDP_SEND_MSG_SIZE
{
    ...
    if (Result != NO_ERROR) {
        int WsaError = WSAGetLastError();
        QuicTraceLogWarning(
            "DatapathQueryUdpSendMsgFailed: [udp] Query for UDP_SEND_MSG_SIZE failed, 0x%x",
            WsaError);
    } else {
    ...
    )
{
    ...
    QuicTraceLogError("DatapathResolveHostNameFailed: [%p] Couldn't resolve hostname '%s' to an IP address",
        Datapath,
        HostName);
    ...
    )
{
    ...
    if (InterlockedDecrement16(
        ...
        QuicTraceLogVerbose(
            "DatapathShutDownComplete: [udp][%p] Shut down (complete)",
    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "DatapathUnreachableWithError: [udp][%p] Received unreachable error (0x%x)!",
        SocketContext->Binding,
        ErrorCode);
    ...
    )
{
    ...
    if (IoResult == WSAENOTSOCK || IoResult == WSA_OPERATION_ABORTED) {
        ...
        QuicTraceLogVerbose(
            "DatapathTooLarge: [udp][%p] Received larger than expected datagram from addr:%p, Length:%d",
            SocketContext->Binding,
        ...
        if (!FoundLocalAddr) {
            ...
            //
            QuicTraceLogWarning(
                "DatapathMissingInfo: [udp][%p] WSARecvMsg completion is missing IP_PKTINFO",
                SocketContext->Binding);
            goto Drop;
        }
        ...
        if (NumberOfBytesTransferred == 0) {
            QuicTraceLogWarning(
                "DatapathRecvEmpty: [udp][%p] Dropping datagram with empty payload.",
                SocketContext->Binding);
            goto Drop;
        }
        ...
        for ( ;
            ...
            *DatagramChainTail = Datagram;
            ...
            if (IsCoalesced && ++MessageCount == URO_MAX_DATAGRAMS_PER_INDICATION) {
                QuicTraceLogWarning(
                    "DatapathUroPreallocExceeded: [udp][%p] Exceeded URO preallocation capacity.",
                    SocketContext->Binding);
                break;
            }
        }
    ...
    )
{
    ...
    QuicTraceLogInfo(
        "DatapathWorkerThreadStart: [udp][%p] Worker start",
        ProcContext);
    ...
    QuicTraceLogInfo(
        "DatapathWorkerThreadStop: [udp][%p] Worker stop",
        ProcContext);
