    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "CertCapiParsedChain: [cert] Successfully parsed chain of %u certificate(s)",
        CertNumber);
...
Exit:
    ...
    QuicTraceLogVerbose(
        "CertCapiFormattedChain: [cert] Successfully formatted chain of %u certificate(s)",
        CertNumber);
...
Exit:
    ...
    QuicTraceLogInfo(
        "CertCapiVerifiedChain: CertVerifyChain: %S 0x%x, result=0x%x",
        ServerName,
        IgnoreFlags,
        Status);
    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "CertCapiVerify: [cert] QuicCertVerify alg=0x%4.4x",
        SignatureAlgorithm);
    ...
    QUIC_SIGN_PADDING Padding;
	QuicZeroMemory(&Padding, sizeof(QUIC_SIGN_PADDING));
