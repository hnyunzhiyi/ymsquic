...
class DatapathHooks
{
    ...
    void Register() {
        ...
        QuicTraceLogInfo(
            "TestHookRegister: [test][hook] Registering");
    ...
    void Unregister() {
#if QUIC_TEST_DATAPATH_HOOKS_ENABLED
        QuicTraceLogInfo(
            "TestHookUnregistering: [test][hook] Unregistering");
        ...
        QuicTraceLogInfo(
            "TestHookUnregistered: [test][hook] Unregistered");
#endif
    }
...
struct RandomLossHelper : public DatapathHook
{
    ...
    Receive(
        ...
        if (Result) {
            QuicTraceLogVerbose(
                "TestHookDropPacketRandom: [test][hook] Random packet drop");
        }
        return Result;
    }
};
...
struct SelectiveLossHelper : public DatapathHook
{
    ...
    Receive(
        ...
        QuicTraceLogVerbose(
            "TestHookDropPacketSelective: [test][hook] Selective packet drop");
        DropPacketCount--;
        return TRUE;
    }
};
...
struct ReplaceAddressHelper : public DatapathHook
{
    ...
    Receive(
        ...
        if (QuicAddrCompare(
            ...
            QuicTraceLogVerbose(
                "TestHookReplaceAddrRecv: [test][hook] Recv Addr :%hu => :%hu",
    ...
    Send(
        ...
        if (QuicAddrCompare(RemoteAddress, &New)) {
            *RemoteAddress = Original;
            QuicTraceLogVerbose(
                "TestHookReplaceAddrSend: [test][hook] Send Addr :%hu => :%hu",
                QuicAddrGetPort(&New),
                QuicAddrGetPort(&Original));
        } else if (QuicAddrCompare(RemoteAddress, &Original)) {
            QuicTraceLogVerbose(
                "TestHookDropOldAddrSend: [test][hook] Dropping send to old addr");
...
struct ReplaceAddressThenDropHelper : public DatapathHook
{
    ...
    Receive(
        ...
        if (QuicAddrCompare(
            ...
            if (AllowPacketCount == 0) {
                QuicTraceLogVerbose(
                    "TestHookDropLimitAddrRecv: [test][hook] Dropping recv over limit to new addr");
                return TRUE; // Drop
            }
            AllowPacketCount--;
            Datagram->Tuple->RemoteAddress = New;
    ...
    Send(
        ...
        if (QuicAddrCompare(RemoteAddress, &New)) {
            if (AllowPacketCount == 0) {
                QuicTraceLogVerbose(
                    "TestHookDropLimitAddrSend: [test][hook] Dropping send over limit to new addr");
                return TRUE; // Drop
            }
            AllowPacketCount--;
            *RemoteAddress = Original;
            QuicTraceLogVerbose(
                "TestHookDropLimitAddrSend: [test][hook] Send Addr :%hu => :%hu",
                QuicAddrGetPort(&New),
                QuicAddrGetPort(&Original));
        } else if (QuicAddrCompare(RemoteAddress, &Original)) {
            QuicTraceLogVerbose(
