...
Error:
    ...
    if (Status != QUIC_STATUS_SUCCESS){
        QuicTraceLogError("ApiExitStatus: ListenerOpen:[api] Exit %u", Status);
    }
    return Status;
}
...
#pragma prefast(suppress: __WARNING_25024, "Pointer cast already validated.")
    ...
    QuicTraceLogVerbose("ListenerDestroyed: [list][%p] Destroyed", Listener);
    ...
    )
{
    ...
    QuicZeroMemory(&BindingLocalAddress, sizeof(QUIC_ADDR));
...
#pragma prefast(suppress: __WARNING_25024, "Pointer cast already validated.")
    ...
    QuicTraceLogVerbose(
        "ListenerStarted: [list][%p] Started, Binding=%p, LocalAddr(addr:%p, Length:%lu)",
        Listener,
        Listener->Binding,
        &Listener->LocalAddress, sizeof(Listener->LocalAddress));
...
Exit:
    ...
    if (QUIC_FAILED(Status)) {
    	QuicTraceLogError("ApiExitStatus: ListenerStart: [api] Exit %u", Status);
    }
...
#pragma prefast(suppress: __WARNING_25024, "Pointer cast already validated.")
        ...
        if (Listener->Binding != NULL) {
            ...
            QuicTraceLogVerbose(
                "ListenerStopped: [list][%p] Stopped",
                Listener);
        }
    }

    QuicTraceLogVerbose("[api] Exit");
}
...
{
    QuicTraceLogVerbose(
        "ListenerRundown: [list][%p] Rundown, Registration=%p",
        Listener,
        Listener->Registration);
    if (Listener->Binding != NULL) {
        QuicTraceLogVerbose(
    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "ListenerIndicateNewConnection: [list][%p] Indicating NEW_CONNECTION",
        Listener);
