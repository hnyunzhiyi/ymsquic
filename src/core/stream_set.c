    ...
    )
{
    ...
    QuicTraceLogConnVerbose(
        "IndicateStreamsAvailable: Indicating QUIC_CONNECTION_EVENT_STREAMS_AVAILABLE [%hu] [%hu]",
        Event.STREAMS_AVAILABLE.BidirectionalCount,
        Event.STREAMS_AVAILABLE.UnidirectionalCount);
    ...
    )
{
    ...
    if (MaxStreams > Info->MaxTotalStreamCount) {
        ...
        QuicTraceLogConnVerbose(
            "PeerStreamCountsUpdated: Peer updated max stream count (%hhu, %lu).",
    ...
    )
{
    ...
    QuicTraceLogConnInfo(
        "MaxStreamCountUpdated: App configured max stream count of %hu (type=%hhu).",
        Count, Type);
    ...
    *ProtocolViolation = FALSE;
    ...
    if (StreamCount <= Info->TotalStreamCount) {
        ...
        do {
            ...
            QuicTraceLogConnVerbose(
                "IndicatePeerStreamStarted: Indicating QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED [%p, 0x%x]",
            ...
            if (QUIC_FAILED(Status)) {
                QuicTraceLogStreamWarning(
                    "NotAccepted: New stream wasn't accepted, 0x%x",
                    Status);
