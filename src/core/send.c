    ...
    )
{
    ...
    if (Send->DelayedAckTimerActive) {
        QuicTraceLogConnVerbose("CancelAckDelayTimer: Canceling ACK_DELAY timer");
    ...
    )
{
    if (!Send->FlushOperationPending && QuicSendCanSendFlagsNow(Send)) {
        ...
        if ((Oper = QuicOperationAlloc(Connection->Worker, QUIC_OPER_TYPE_FLUSH_SEND)) != NULL) {
            ...
            QuicTraceLogVerbose(
                "ConnQueueSendFlush: [conn][%p] Queueing send flush, reason=%u",
                Connection,
                Reason);
    ...
    )
{
    ...
    if (SendFlags & QUIC_CONN_SEND_FLAG_ACK && Send->DelayedAckTimerActive) {
        QuicTraceLogConnVerbose(
            "CancelAckDelayTimer: Canceling ACK_DELAY timer");
    ...
    if (CanSetFlag && (Send->SendFlags & SendFlags) != SendFlags) {
        QuicTraceLogConnVerbose(
            "ScheduleSendFlags: Scheduling flags 0x%x to 0x%x",
            SendFlags,
            Send->SendFlags);
    ...
    )
{
    if (Send->SendFlags & SendFlags) {
        QuicTraceLogConnVerbose(
            "RemoveSendFlags: Removing flags %x",
    ...
    )
{
    ...
    if (Stream->SendFlags & SendFlags) {
        ...
        QuicTraceLogStreamVerbose(
            "ClearSendFlags: Removing flags %x",
    ...
    )
{
    ...
    if (Send->SendFlags & QUIC_CONN_SEND_FLAG_PATH_RESPONSE) {
        ...
        for (i = 0; i < Connection->PathsCount; ++i) {
	    	...
	    	QuicZeroMemory(&Frame, sizeof(QUIC_PATH_RESPONSE_EX));
    ...
    )
{
    ...
    for (uint8_t i = 0; i < Connection->PathsCount; ++i) {
		...
		QuicZeroMemory(&Builder, sizeof(QUIC_PACKET_BUILDER));
    ...
    )
{
    ...
    do {
        ...
        if (Path->Allowance < QUIC_MIN_SEND_ALLOWANCE) {
            QuicTraceLogConnVerbose(
                "AmplificationProtectionBlocked: Cannot send any more because of amplification protection");
        ...
        if (!QuicPacketBuilderHasAllowance(&Builder)) {
            ...
            if (!SendFlags) {
                if (QuicCongestionControlCanSend(&Connection->CongestionControl)) {
                    ...
                    QuicTraceLogConnVerbose(
                        "SetPacingTimer: Setting delayed send (PACING) timer for %u ms",
    ...
    QuicTraceLogConnVerbose(
        "SendFlushComplete: Flush complete flags=0x%x",
        Send->SendFlags);
    ...
    )
{
    ...
    if (!Send->DelayedAckTimerActive &&
        ...
        QuicTraceLogConnVerbose(
            "StartAckDelayTimer: Starting ACK_DELAY timer for %u ms",
