    ...
    )
{
    ...
    QuicTraceLogVerbose("ConnCubic: [conn][%p] CUBIC: SlowStartThreshold=%u K=%u WindowMax=%u WindowLastMax=%u",
        Connection,
...
{
    QUIC_CONNECTION* Connection = QuicCongestionControlGetConnection(Cc);
    QuicTraceLogVerbose("ConnCongestion: [conn][%p] Congestion event", Connection);
...
{
    QUIC_CONNECTION* Connection = QuicCongestionControlGetConnection(Cc);
    QuicTraceLogVerbose("ConnPersistentCongestion: [conn][%p] Persistent congestion event", Connection);
    ...
    )
{
    ...
    if (Cc->IsInRecovery) {
        if (LargestPacketNumberAcked > Cc->RecoverySentPacketNumber) {
            ...
            //
            QuicTraceLogVerbose("ConnRecoveryExit: [conn][%p] Recovery complete", Connection);
            Cc->IsInRecovery = FALSE;
