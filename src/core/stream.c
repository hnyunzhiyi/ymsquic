    ...
    )
{
    ...
    QUIC_TEL_ASSERT(Connection->State.ClosedLocally || Stream->Flags.ShutdownComplete);
    QUIC_TEL_ASSERT(Connection->State.ClosedLocally || Stream->Flags.HandleClosed);
...
#pragma warning(disable:6001) // SAL doesn't understand we're logging just the address
        QuicTraceLogVerbose("StreamDestroyed: [strm][%p] Destroyed", Stream);
#pragma warning(pop)
    }
}
    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "StreamCreated: [strm][%p] Created, Conn=%p ID=%lu IsLocal=%hhu",
        Stream,
        Stream->Connection,
        Stream->ID,
        !IsRemoteStream);
    ...
    if (Stream->OutFlowBlockedReasons != 0) {
        QuicTraceLogVerbose(
            "StreamOutFlowBlocked: [strm][%p] Send Blocked Flags: %hhu",
            Stream,
...
{
    QUIC_DBG_ASSERT(!Stream->Flags.HandleClosed);
    ...
    if (!Stream->Flags.ShutdownComplete) {
        ...
        if (Stream->Flags.Started) {
            ...
            QuicTraceLogStreamWarning(
                "CloseWithoutShutdown: Closing handle without fully shutting down");
        }
...
{
    QuicTraceLogVerbose(
        "StreamRundown: [strm][%p] Rundown, Conn=%p ID=%lu IsLocal=%lu",
        Stream,
        Stream->Connection,
        Stream->ID,
    ...
    )
{
    ...
    if (Stream->ClientCallbackHandler != NULL) {
        ...
        QuicTraceLogStreamWarning(
            "EventSilentDiscard: Event silently discarded");
    }
    return Status;
}
    ...
    )
{
    ...
    QuicTraceLogStreamVerbose(
        "IndicateStartComplete: Indicating QUIC_STREAM_EVENT_START_COMPLETE (0x%x)",
        Status);
    ...
    )
{
    if (!Stream->Flags.HandleShutdown) {
        ...
        QuicTraceLogStreamVerbose(
            "IndicateStreamShutdownComplete: Indicating QUIC_STREAM_EVENT_SHUTDOWN_COMPLETE");
        (void)QuicStreamIndicateEvent(Stream, &Event);
