    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "RegistrationCreated: [reg][%p] Created, AppName=%s",
        Registration,
...
Error:
	...
	if (QUIC_FAILED(Status)){
    	    QuicTraceLogError("ApiExitStatus: [api] Exit Error:%u", Status);
	}
...
#pragma prefast(suppress: __WARNING_25024, "Pointer cast already validated.")
        ...
        QuicTraceLogVerbose(
            "RegistrationCleanup: [reg][%p] Cleaning up",
            Registration);
...
{
    QuicTraceLogVerbose(
        "RegistrationRundown: [reg][%p] Rundown, AppName=%s",
        Registration,
        Registration->AppName);
