    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "ConnOutFlowStats: [conn][%p] OUT: BytesSent=%lu InFlight=%u InFlightMax=%u CWnd=%u SSThresh=%u ConnFC=%lu ISB=%lu PostedBytes=%lu SRtt=%u",
        Connection,
    ...
    QuicTraceLogVerbose(
        "ConnOutFlowStreamStats: [conn][%p] OUT: StreamFC=%lu StreamSendWindow=%lu",
        Connection,
        FcAvailable,
        SendWindow);
}
    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "ConnInFlowStats: [conn][%p] IN: BytesRecv=%lu",
        Connection,
    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "ConnStats: [conn][%p] STATS: SRtt=%u CongestionCount=%u PersistentCongestionCount=%u SendTotalBytes=%lu RecvTotalBytes=%lu",
        Connection,
        Path->SmoothedRtt,
    ...
    QuicTraceLogVerbose(
        "ConnPacketStats: [conn][%p] STATS: SendTotalPackets=%lu SendSuspectedLostPackets=%lu SendSpuriousLostPackets=%lu RecvTotalPackets=%lu RecvReorderedPackets=%lu RecvDroppedPackets=%lu RecvDuplicatePackets=%lu RecvDecryptionFailures=%lu",
        Connection,
    ...
    )
{
    if (!(Connection->OutFlowBlockedReasons & Reason)) {
        ...
        QuicTraceLogVerbose(
            "ConnOutFlowBlocked: [conn][%p] Send Blocked Flags: %hhu",
            Connection,
        ...
        if (SourceCid->CID.SequenceNumber == SequenceNumber) {
            if (RemoveFromList) {
                QuicBindingRemoveSourceConnectionID(
                    ...
                    SourceCid,
                    Entry);
                QuicTraceLogVerbose("ConnSourceCidRemoved: [conn][%p] (SeqNum=%lu) Removed Source CID(addr:%p, Length:%d)",
                    Connection,
