    ...
    )
{
    ...
    if (!Stream->Flags.HandleSendShutdown) {
        ...
        QuicTraceLogStreamVerbose(
            "IndicateSendShutdownComplete: Indicating QUIC_STREAM_EVENT_SEND_SHUTDOWN_COMPLETE");
...
Exit:
    ...
    QuicTraceLogVerbose(
        "StreamSendState: [strm][%p] Send State: %hhu",
        Stream, QuicStreamSendGetState(Stream));
    ...
    )
{
    ...
    if (!(SendRequest->Flags & QUIC_SEND_FLAG_BUFFERED)) {
        ...
        if (Canceled) {
            QuicTraceLogStreamVerbose(
                "IndicateSendCanceled: Indicating QUIC_STREAM_EVENT_SEND_COMPLETE [%p] (Canceled)",
                SendRequest);
        } else {
            QuicTraceLogStreamVerbose(
                "IndicateSendComplete: Indicating QUIC_STREAM_EVENT_SEND_COMPLETE [%p]",
                SendRequest);
        }
    ...
    )
{
    ...
    while (ApiSendRequests != NULL) {
        ...
        *Stream->SendRequestsTail = SendRequest;
        ...
        QuicTraceLogStreamVerbose(
            "SendQueued: Send Request [%p] queued with %lu bytes at offset %lu (flags 0x%x)",
            SendRequest,
    ...
    )
{
    ...
    if (*FrameBytes < HeaderLength) {
        QuicTraceLogStreamVerbose(
            "NoMoreRoom: Can't squeeze in a frame (no room for header)");
        *FramePayloadBytes = 0;
        *FrameBytes = 0;
        return;
    }
    ...
    if ((Stream->SendFlags & QUIC_STREAM_SEND_FLAG_FIN) &&
        ...
        //
        QuicTraceLogStreamVerbose("NoMoreFrames: No more frames");
        *FramePayloadBytes = 0;
        *FrameBytes = 0;
        return;
    }
    ...
    QuicTraceLogStreamVerbose(
        "AddFrame: Built stream frame, offset=%lu len=%d fin=%hhu",
        Frame.Offset,
    ...
    )
{
    ...
    if ((FrameMetadata->Flags & QUIC_SENT_FRAME_FLAG_STREAM_OPEN) &&
        ...
        QuicTraceLogStreamVerbose(
            "RecoverOpen: Recovering open STREAM frame");
    }
    ...
    if ((FrameMetadata->Flags & QUIC_SENT_FRAME_FLAG_STREAM_FIN) &&
        ...
        QuicTraceLogStreamVerbose(
            "RecoverFin: Recovering fin STREAM frame");
    }
    ...
    if (UpdatedRecoveryWindow) {
        ...
        QuicTraceLogStreamVerbose(
            "RecoverRange: Recovering offset %lu up to %lu",
            Start, End);
    ...
    )
{
    ...
    QuicTraceLogStreamVerbose(
        "AckRange: Received ack for %d bytes, offset=%lu, FF=0x%hx",
        Length,
        Offset,
        FrameMetadata->Flags);
    ...
    if (PacketFlags.KeyType == QUIC_PACKET_KEY_0_RTT &&
        ...
        Stream->Sent0Rtt = FollowingOffset;
        QuicTraceLogStreamVerbose(
            "Send0RttUpdated: Updated sent 0RTT length to %lu",
            FollowingOffset);
    }
    ...
    if (Offset <= Stream->UnAckedOffset) {
        ...
        if (Stream->UnAckedOffset == Stream->QueuedSendOffset && Stream->Flags.FinAcked) {
            ...
            QuicTraceLogStreamVerbose("SendQueueDrained: Send queue completely drained");
    ...
    )
{
    if (QuicTraceLogStreamVerboseEnabled()) {
        ...
        QuicTraceLogStreamVerbose(
            "SendDump: SF:%hX FC:%lu QS:%lu MAX:%lu UNA:%lu NXT:%lu RECOV:%lu-%lu",
            Stream->SendFlags,
