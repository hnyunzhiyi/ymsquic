    ...
    )
{
    ...
    if (TimerWheel->Slots == NULL) {
        QuicTraceLogError(
            "AllocFailure: Allocation of '%s' failed. (%lu bytes)", "timerwheel slots",
        ...
        for (uint32_t i = 0; i < TimerWheel->SlotCount; ++i) {
            ...
            while (Entry != ListHead) {
                ...
                QuicTraceLogConnWarning(
                    "StillInTimerWheel: Still in timer wheel! Connection was likely leaked!");
                Entry = Entry->Flink;
            }
    ...
    )
{
    ...
    if (NewSlots == NULL) {
        QuicTraceLogError(
            "AllocFailure: Allocation of '%s' failed. (%lu bytes)", "timerwheel slots (realloc)",
            NewSlotCount * sizeof(QUIC_LIST_ENTRY));
        return;
    }

    QuicTraceLogVerbose(
        "TimerWheelResize: [time][%p] Resizing timer wheel (new slot count = %u).",
        TimerWheel,
        NewSlotCount);
    ...
    )
{
    ...
    if (TimerWheel->NextConnection == NULL) {
        QuicTraceLogVerbose(
            "TimerWheelNextExpirationNull: [time][%p] Next Expiration = {NULL}.",
            TimerWheel);
    } else {
        QuicTraceLogVerbose(
            "TimerWheelNextExpiration: [time][%p] Next Expiration = {%lu, %p}.",
            TimerWheel,
    ...
    )
{
    if (Connection->TimerLink.Flink != NULL) {
        ...
        //
        QuicTraceLogVerbose(
            "TimerWheelRemoveConnection: [time][%p] Removing Connection %p.",
            TimerWheel,
            Connection);
    ...
    )
{
    ...
    if (ExpirationTime == UINT64_MAX) {
        ...
        QuicTraceLogVerbose(
            "TimerWheelUpdateConnection: [time][%p] Updating Connection %p.",
            TimerWheel, Connection);
