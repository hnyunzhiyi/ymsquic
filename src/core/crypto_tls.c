    ...
    )
{
    ...
    if (Info->ServerName == NULL) {
        QuicTraceLogConnWarning("NoSniPresent: No SNI extension present");
    }
    ...
    )
{
    ...
    QuicTraceLogConnVerbose(
        "EncodeTPStart: Encoding Transport Parameters (Server = %hhu)",
        IsServerTP);
    ...
    *TPLen = (uint32_t)(QuicTlsTPHeaderSize + RequiredTPLen);
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_ORIGINAL_DESTINATION_CONNECTION_ID) {
        ...
        TPBuf =
            TlsWriteTransportParam(
                ...
                TPBuf);
        QuicTraceLogConnVerbose(
            "EncodeTPOriginalDestinationCID: TP Original Destination Connection ID (%s)",
            QuicCidBufToStr(
                TransportParams->OriginalDestinationConnectionID,
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_IDLE_TIMEOUT) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPIdleTimeout: TP Idle Timeout (%lu ms)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_STATELESS_RESET_TOKEN) {
        ...
        TPBuf =
            TlsWriteTransportParam(
                ...
                TPBuf);
        QuicTraceLogConnVerbose(
            "EncodeTPStatelessResetToken: TP Stateless Reset Token (%s)",
            QuicCidBufToStr(
                TransportParams->StatelessResetToken,
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_MAX_UDP_PAYLOAD_SIZE) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPMaxUdpPayloadSize: TP Max Udp Payload Size (%lu bytes)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_INITIAL_MAX_DATA) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPInitMaxData: TP Max Data (%lu bytes)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_INITIAL_MAX_STRM_DATA_BIDI_LOCAL) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPInitMaxStreamDataBidiLocal: TP Max Local Bidirectional Stream Data (%lu bytes)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_INITIAL_MAX_STRM_DATA_BIDI_REMOTE) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPInitMaxStreamDataBidiRemote: TP Max Remote Bidirectional Stream Data (%lu bytes)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_INITIAL_MAX_STRM_DATA_UNI) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPInitMaxStreamUni: TP Max Unidirectional Stream Data (%lu)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_INITIAL_MAX_STRMS_BIDI) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPMaxBidiStreams: TP Max Bidirectional Streams (%lu)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_INITIAL_MAX_STRMS_UNI) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPMaxUniStreams: TP Max Unidirectional Streams (%lu)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_ACK_DELAY_EXPONENT) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPAckDelayExponent: TP ACK Delay Exponent (%lu)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_MAX_ACK_DELAY) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPMaxAckDelay: TP Max ACK Delay (%lu ms)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_DISABLE_ACTIVE_MIGRATION) {
        TPBuf =
            TlsWriteTransportParam(
                ...
                TPBuf);
        QuicTraceLogConnVerbose(
            "EncodeTPDisableMigration: TP Disable Active Migration");
    }
    if (TransportParams->Flags & QUIC_TP_FLAG_PREFERRED_ADDRESS) {
        QUIC_DBG_ASSERT(IsServerTP);
        QUIC_FRE_ASSERT(FALSE); // TODO - Implement
        QuicTraceLogConnVerbose(
            "EncodeTPPreferredAddress: TP Preferred Address");
    }
    if (TransportParams->Flags & QUIC_TP_FLAG_ACTIVE_CONNECTION_ID_LIMIT) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPCIDLimit: TP Connection ID Limit (%lu)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_INITIAL_SOURCE_CONNECTION_ID) {
        TPBuf =
            TlsWriteTransportParam(
                ...
                TPBuf);
        QuicTraceLogConnVerbose(
            "EncodeTPOriginalCID: TP Initial Source Connection ID (%s)",
            QuicCidBufToStr(
                TransportParams->InitialSourceConnectionID,
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_RETRY_SOURCE_CONNECTION_ID) {
        ...
        TPBuf =
            TlsWriteTransportParam(
                ...
                TPBuf);
        QuicTraceLogConnVerbose(
            "EncodeTPRetrySourceCID: TP Retry Source Connection ID (%s)",
            QuicCidBufToStr(
                TransportParams->RetrySourceConnectionID,
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_MAX_DATAGRAM_FRAME_SIZE) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeMaxDatagramFrameSize: TP Max Datagram Frame Size (%lu bytes)",
    ...
    if (TransportParams->Flags & QUIC_TP_FLAG_DISABLE_1RTT_ENCRYPTION) {
        TPBuf =
            TlsWriteTransportParam(
                ...
                TPBuf);
        QuicTraceLogConnVerbose(
            "EncodeTPDisable1RttEncryption: TP Disable 1-RTT Encryption");
    }
    if (TestParam != NULL) {
        TPBuf =
            TlsWriteTransportParam(
                ...
                TPBuf);
        QuicTraceLogConnVerbose(
            "EncodeTPTest: TP TEST TP (Type %hu, Length %hu)",
            TestParam->Type,
            TestParam->Length);
    }
    ...
    if (FinalTPLength != RequiredTPLen) {
        ...
        QuicTraceLogConnVerbose(
            "EncodeTPEnd: Encoded %hu bytes for QUIC TP",
    ...
    )
{
    ...
    QuicTraceLogConnVerbose(
        "DecodeTPStart: Decoding Transport Parameters (Server = %hhu) (%hu bytes)",
        IsServerTP,
        TPLen);
    ...
    while (Offset < TPLen) {
		...
		QuicZeroMemory(&ParamLength, sizeof(QUIC_VAR_INT));
    ...
    #define TRY_READ_VAR_INT(Param) \
        ...
        case QUIC_TP_ID_ORIGINAL_DESTINATION_CONNECTION_ID:
            ...
            QuicCopyMemory(
                ...
                Length);
            QuicTraceLogConnVerbose(
                "DecodeTPOriginalDestinationCID: TP Original Connection Destination ID (%s)",
                QuicCidBufToStr(
                    TransportParams->OriginalDestinationConnectionID,
                    TransportParams->OriginalDestinationConnectionIDLength).Buffer);
            break;
        ...
        case QUIC_TP_ID_IDLE_TIMEOUT:
            ...
            QuicTraceLogConnVerbose(
                "DecodeTPIdleTimeout: TP Idle Timeout (%lu ms)",
                TransportParams->IdleTimeout);
            break;
        ...
        case QUIC_TP_ID_STATELESS_RESET_TOKEN:
            if (Length != QUIC_STATELESS_RESET_TOKEN_LENGTH) {
                QuicTraceLogError(
                    "DecodeTPIdleTimeout: [conn][%p] ERROR, %u, %s.",
                    Connection,
                    Length,
        ...
        case QUIC_TP_ID_MAX_UDP_PAYLOAD_SIZE:
            ...
            QuicTraceLogConnVerbose(
                "DecodeTPMaxUdpPayloadSize: TP Max Udp Payload Size (%lu bytes)",
                TransportParams->MaxUdpPayloadSize);
            break;
        ...
        case QUIC_TP_ID_INITIAL_MAX_DATA:
            ...
            QuicTraceLogConnVerbose(
                "DecodeTPInitMaxData: TP Max Data (%lu bytes)",
                TransportParams->InitialMaxData);
            break;
        ...
        case QUIC_TP_ID_INITIAL_MAX_STREAM_DATA_BIDI_LOCAL:
            ...
            QuicTraceLogConnVerbose(
                "ConnErrorStatus: TP Max Local Bidirectional Stream Data (%lu bytes)",
                TransportParams->InitialMaxStreamDataBidiLocal);
            break;
        ...
        case QUIC_TP_ID_INITIAL_MAX_STREAM_DATA_BIDI_REMOTE:
            ...
            QuicTraceLogConnVerbose(
                "DecodeTPInitMaxStreamDataBidiRemote: TP Max Remote Bidirectional Stream Data (%lu bytes)",
                TransportParams->InitialMaxStreamDataBidiRemote);
            break;
        ...
        case QUIC_TP_ID_INITIAL_MAX_STREAM_DATA_UNI:
            ...
            QuicTraceLogConnVerbose(
                "DecodeTPInitMaxStreamDataBidiUni: TP Max Unidirectional Stream Data (%lu)",
                TransportParams->InitialMaxStreamDataUni);
            break;
        ...
        case QUIC_TP_ID_INITIAL_MAX_STREAMS_BIDI:
            ...
            QuicTraceLogConnVerbose(
                "DecodeTPMaxBidiStreams: TP Max Bidirectional Streams (%lu)",
                TransportParams->InitialMaxBidiStreams);
            break;
        ...
        case QUIC_TP_ID_INITIAL_MAX_STREAMS_UNI:
            ...
            QuicTraceLogConnVerbose(
                "TP: Max Unidirectional Streams (%lu)",
                TransportParams->InitialMaxUniStreams);
            break;
        ...
        case QUIC_TP_ID_ACK_DELAY_EXPONENT:
            ...
            QuicTraceLogConnVerbose(
                "DecodeTPAckDelayExponent: TP ACK Delay Exponent (%lu)",
                TransportParams->AckDelayExponent);
            break;
        ...
        case QUIC_TP_ID_MAX_ACK_DELAY:
            ...
            QuicTraceLogConnVerbose(
                "DecodeTPMaxAckDelay: TP Max ACK Delay (%lu ms)",
                TransportParams->MaxAckDelay);
            break;
        ...
        case QUIC_TP_ID_ACTIVE_CONNECTION_ID_LIMIT:
            ...
            QuicTraceLogConnVerbose(
                "DecodeTPCIDLimit: TP Connection ID Limit (%lu)",
                TransportParams->ActiveConnectionIdLimit);
            break;
        ...
        case QUIC_TP_ID_MAX_DATAGRAM_FRAME_SIZE:
            ...
            QuicTraceLogConnVerbose(
                "TP: Max Datagram Frame Size (%lu bytes)",
                TransportParams->MaxDatagramFrameSize);
            break;
        ...
        case QUIC_TP_ID_DISABLE_1RTT_ENCRYPTION:
            ...
            QuicTraceLogConnVerbose(
                "DecodeTPDisable1RttEncryption: TP Disable 1-RTT Encryption");
            break;

        default:
            if (QuicTpIdIsReserved(Id)) {
                QuicTraceLogConnWarning(
                    "DecodeTPReserved: TP Reserved ID %lu, length %hu",
                    Id,
                    Length);
            } else {
                QuicTraceLogConnWarning(
                    "DecodeTPUnknown: TP Unknown ID %lu, length %hu",
                    Id,
                    Length);
            }
            break;
