...
QUIC_STATIC_ASSERT(QUIC_MAX_MTU >= QUIC_DPLPMUTD_DEFAULT_MAX_MTU, L"Default max must not be more than max");
QUIC_STATIC_ASSERT(QUIC_DPLPMUTD_MIN_MTU <= QUIC_DPLPMUTD_DEFAULT_MIN_MTU, L"Default min must not be more than min");
...
{
    QUIC_PATH* Path =
        QUIC_CONTAINING_RECORD(MtuDiscovery, QUIC_PATH, MtuDiscovery);
    MtuDiscovery->IsSearchComplete = TRUE;
    MtuDiscovery->SearchCompleteEnterTimeUs = QuicTimeUs64();
    ...
    )
{
    ...
    QUIC_DBG_ASSERT(Path->Mtu <= MtuDiscovery->MaxMtu);
