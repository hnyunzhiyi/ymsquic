    ...
    )
{
    ...
    QuicTraceLogConnVerbose(
        "DatagramSendStateChanged: Indicating DATAGRAM_SEND_STATE_CHANGED to %u",
        State);
    ...
    )
{
    ...
    QuicTraceLogConnVerbose("DatagramSendShutdown: Datagram send shutdown");
    ...
    )
{
    ...
    while (ApiQueue != NULL) {
        ...
        QuicTraceLogConnVerbose(
            "DatagramSendQueued: Datagram [%p] queued with %lu bytes (flags 0x%x)",
            SendRequest,
