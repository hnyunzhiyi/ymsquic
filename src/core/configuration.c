    ...
    )
{
    ...
    QuicTraceLogVerbose("ApiEnter: [api] Enter %u (%p).", QUIC_TRACE_API_CONFIGURATION_OPEN, Handle);
    ...
    if (Configuration == NULL) {
        QuicTraceLogError("AllocFailure: Allocation of '%s' failed. (%lu bytes)", "QUIC_CONFIGURATION", sizeof(QUIC_CONFIGURATION));
    ...
    if (Configuration->Silo != NULL) {
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceLogWarning("ConfigurationOpenStorageFailed:[cnfg][%p] Failed to open settings, 0x%x", Configuration, Status);
    ...
    if (Registration->AppNameLength != 0) {
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceLogInfo(
                "ConfigurationOpenAppStorageFailed:[cnfg][%p] Failed to open app specific settings, 0x%x",
                Configuration,
                Status);
            Status = QUIC_STATUS_SUCCESS; // Non-fatal, as the process may not have access
        }
    ...
    QuicTraceLogVerbose("ConfigurationCreated:[cnfg][%p] Created, Registration=%p", Configuration, Registration);
    ...
    )
{
    ...
    QuicTraceLogVerbose("ConfigurationCleanup: [cnfg][%p] Cleaning up", Configuration);
...
#pragma prefast(suppress: __WARNING_25024, "Pointer cast already validated.")
    ...
    QuicTraceLogVerbose("ApiExitStatus: [api] Exit %u", Status);
...
{
    QuicTraceLogVerbose("ConfigurationRundown: [cnfg][%p] Rundown, Registration=%p",
        Configuration,
    ...
    )
{
    ...
    QuicTraceLogInfo("ConfigurationSettingsUpdated: [cnfg][%p] Settings %p Updated",
        Configuration,
        &Configuration->Settings);
