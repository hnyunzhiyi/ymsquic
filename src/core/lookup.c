    ...
    )
{
    ...
    if (Connection != NULL) {
        QuicTraceLogVerbose(
            "LookupCidFound: [look][%p] Lookup Hash=%u found %p",
            Lookup,
            Hash,
            Connection);
    } else {
        QuicTraceLogVerbose(
            "LookupCidNotFound: [look][%p] Lookup Hash=%u not found",
            Lookup,
            Hash);
    }
#endif
...
#if QUIC_DEBUG_HASHTABLE_LOOKUP
            QuicTraceLogVerbose(
                "LookupRemoteHashFound: [look][%p] Lookup RemoteHash=%u found %p",
                Lookup,
                Hash,
                Connection);
#endif
...
#if QUIC_DEBUG_HASHTABLE_LOOKUP
    QuicTraceLogVerbose(
        "LookupRemoteHashNotFound: [look][%p] Lookup RemoteHash=%u not found",
        Lookup,
        Hash);
#endif
...
#if QUIC_DEBUG_HASHTABLE_LOOKUP
    QuicTraceLogVerbose(
        "LookupCidInsert: [look][%p] Insert Conn=%p Hash=%u",
        Lookup,
        Connection,
        Hash);
#endif
...
#if QUIC_DEBUG_HASHTABLE_LOOKUP
    QuicTraceLogVerbose(
        "LookupRemoteHashInsert: [look][%p] Insert Conn=%p RemoteHash=%u",
        Lookup,
        Connection,
        Hash);
#endif
...
#if QUIC_DEBUG_HASHTABLE_LOOKUP
    QuicTraceLogVerbose(
        "LookupCidRemoved: [look][%p] Remove Conn=%p",
        Lookup,
