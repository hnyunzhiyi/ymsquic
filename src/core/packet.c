    ...
    )
{
    ...
    if (Invariant->IsLongHeader) {
        ...
        case QUIC_VERSION_VER_NEG: {
            QuicTraceLogVerbose(
                "LogPacketVersionNegotiation: [%c][%cX][-] VerNeg DestCid:%s SrcCid:%s (Payload %d bytes)",
            ...
            while (Offset < PacketLength) {
                QuicTraceLogVerbose(
                    "LogPacketVersionNegotiationVersion: [%c][%cX][-]   Ver:0x%x",
                    PtkConnPre(Connection),
                    PktRxPre(Rx),
        ...
        case QUIC_VERSION_MS_1: {
            ...
            if (LongHdr->Type == QUIC_INITIAL) {
                ...
                QuicTraceLogVerbose(
                    "LogPacketRetry: [%c][%cX][-] LH Ver:0x%x DestCid:%s SrcCid:%s Type:R (Token %hu bytes)",
                    PtkConnPre(Connection),
                    PktRxPre(Rx),
                    LongHdr->Version,
                    QuicCidBufToStr(DestCid, DestCidLen).Buffer,
            ...
            if (LongHdr->Type == QUIC_INITIAL) {
                QuicTraceLogVerbose(
                    "LogPacketLongHeaderInitial: [%c][%cX][%lu] LH Ver:0x%x DestCid:%s SrcCid:%s Type:I (Token %hu bytes) (Payload %hu bytes)",
                    PtkConnPre(Connection),
                    PktRxPre(Rx),
                    PacketNumber,
                    LongHdr->Version,
                ...
                QuicTraceLogVerbose(
                    "LogPacketLongHeader: [%c][%cX][%lu] LH Ver:0x%x DestCid:%s SrcCid:%s Type:%s (Payload %hu bytes)",
                    PtkConnPre(Connection),
                    PktRxPre(Rx),
                    PacketNumber,
                    LongHdr->Version,
        ...
        default:
            QuicTraceLogVerbose(
                "LogPacketLongHeaderUnsupported: [%c][%cX][%lu] LH Ver:[UNSUPPORTED,0x%x] DestCid:%s SrcCid:%s",
                PtkConnPre(Connection),
                PktRxPre(Rx),
                PacketNumber,
        ...
        case QUIC_VERSION_MS_1: {
            ...
            QuicTraceLogVerbose(
                "LogPacketShortHeader: [%c][%cX][%lu] SH DestCid:%s KP:%hu SB:%hu (Payload %hu bytes)",
                PtkConnPre(Connection),
                PktRxPre(Rx),
                PacketNumber,
                QuicCidBufToStr(DestCid, DestCidLen).Buffer,
    ...
    )
{
    ...
    if (Packet->AssignedToConnection) {
        ...
        QuicTraceLogVerbose(
            "ConnDropPacket: [conn][%p] DROP packet Dst(addr:%p, Length:%lu) Src(addr:%p, Length:%lu) Reason=%s.",
            Owner,
            &Datagram->Tuple->LocalAddress, sizeof(Datagram->Tuple->LocalAddress),
            &Datagram->Tuple->RemoteAddress, sizeof(Datagram->Tuple->RemoteAddress),
            Reason);
    } else {
        InterlockedIncrement64((int64_t*)&((QUIC_BINDING*)Owner)->Stats.Recv.DroppedPackets);
        QuicTraceLogVerbose(
            "BindingDropPacket: [bind][%p] DROP packet Dst(addr:%p, Length:%lu) Src(addr:%p, Length:%lu) Reason=%s.",
            Owner,
            &Datagram->Tuple->LocalAddress, sizeof(Datagram->Tuple->LocalAddress),
            &Datagram->Tuple->RemoteAddress, sizeof(Datagram->Tuple->RemoteAddress),
            Reason);
    ...
    )
{
    ...
    if (Packet->AssignedToConnection) {
        InterlockedIncrement64((int64_t*)&((QUIC_CONNECTION*)Owner)->Stats.Recv.DroppedPackets);
        QuicTraceLogVerbose(
            "ConnDropPacketEx: [conn][%p] DROP packet Value=%lu Dst(addr:%p, Length:%lu) Src(addr:%p, Length:%lu) Reason=%s.",
            Owner,
            Value,
            &Datagram->Tuple->LocalAddress, sizeof(Datagram->Tuple->LocalAddress),
            &Datagram->Tuple->RemoteAddress, sizeof(Datagram->Tuple->RemoteAddress),
        ...
        InterlockedIncrement64((int64_t*)&((QUIC_BINDING*)Owner)->Stats.Recv.DroppedPackets);
        QuicTraceLogVerbose(
            "BindingDropPacketEx: [bind][%p] DROP packet %lu. Dst(addr:%p, Length:%lu) Src(addr:%p, Length:%lu) Reason=%s",
            Owner,
            Value,
            &Datagram->Tuple->LocalAddress, sizeof(Datagram->Tuple->LocalAddress),
            &Datagram->Tuple->RemoteAddress, sizeof(Datagram->Tuple->RemoteAddress),
