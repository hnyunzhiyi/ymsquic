    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "WorkerCreated: [wrkr][%p] Created, IdealProc=%hu Owner=%p",
        Worker,
        IdealProcessor,
        Owner);
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceLogError(
            "WorkerErrorStatus: [wrkr][%p] ERROR, %u, %s.",
            Worker,
            Status,
            "QuicThreadCreate");
        goto Error;
...
{
    QuicTraceLogVerbose(
        "WorkerCleanup: [wrkr][%p] Cleaning up",
        Worker);
    ...
    QuicTraceLogVerbose(
        "WorkerDestroyed: [wrkr][%p] Destroyed",
        Worker);
}
    ...
    )
{
    ...
    if (!Connection->WorkerProcessing && !Connection->HasQueuedWork) {
        ...
        QuicTraceLogVerbose(
            "ConnScheduleState: [conn][%p] Scheduling: %u",
            Connection,
    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "WorkerActivityStateUpdated: [wrkr][%p] IsActive = %hhu, Arg = %u",
        Worker,
        Worker->IsActive,
        Arg);
}
    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "WorkerQueueDelayUpdated: [wrkr][%p] QueueDelay = %u",
        Worker,
    ...
    )
{
    ...
    if (Connection->State.UpdateWorker) {
        ...
        QuicTraceLogConnVerbose(
            "IndicateIdealProcChanged: Indicating QUIC_CONNECTION_EVENT_IDEAL_PROCESSOR_CHANGED");
...
QUIC_THREAD_CALLBACK(QuicWorkerThread, Context)
{
    ...
    while (!QuicListIsEmpty(&Worker->Connections)) {
        ...
        if (!Connection->State.ExternalOwner) {
            ...
            //
            QuicTraceLogConnVerbose(
                "AbandonOnLibShutdown: Abandoning on shutdown");
    ...
    QuicTraceLogVerbose("WorkerStop: [wrkr][%p] Stop", Worker);
