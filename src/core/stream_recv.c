...
Exit:
    ...
    QuicTraceLogVerbose(
        "StreamRecvState: [strm][%p] Recv State: %hhu",
        Stream,
    ...
    )
{
    ...
    if (Stream->Flags.ReceiveEnabled &&
        ...
        QuicTraceLogStreamVerbose("QueueRecvFlush: Queuing recv flush");
    ...
    )
{
    ...
    if (!Stream->Flags.RemoteCloseAcked) {
        ...
        if (TotalRecvLength > FinalSize) {
            ...
            //
            QuicTraceLogStreamWarning("ResetEarly: Tried to reset at earlier final size!");
            ...
            if (Stream->Connection->Send.OrderedStreamBytesReceived < FlowControlIncrease ||
                ...
                // connection.
                //
                QuicTraceLogStreamWarning("ResetTooBig: Tried to reset with too big final size!");
                QuicConnTransportError(Stream->Connection, QUIC_ERROR_FINAL_SIZE_ERROR);
                return;
            }
        }

        QuicTraceLogVerbose("StreamRecvState: [strm][%p] Recv State: %hhu",
            Stream,
            QuicStreamRecvGetState(Stream));

        if (!Stream->Flags.SentStopSending) {
            QuicTraceLogStreamInfo("RemoteCloseReset: Closed remotely (reset)");

            QUIC_STREAM_EVENT Event;
            Event.Type = QUIC_STREAM_EVENT_PEER_SEND_ABORTED;
            Event.PEER_SEND_ABORTED.ErrorCode = ErrorCode;
            QuicTraceLogStreamVerbose("IndicatePeerSendAbort: Indicating QUIC_STREAM_EVENT_PEER_SEND_ABORTED (0x%lX)",
                ErrorCode);
    ...
    )
{
    if (!Stream->Flags.LocalCloseAcked && !Stream->Flags.LocalCloseReset) {
        ...
        //
        QuicTraceLogStreamInfo("LocalCloseStopSending: Closed locally (stop sending)");
        ...
        QuicTraceLogStreamVerbose("IndicatePeerReceiveAborted: Indicating QUIC_STREAM_EVENT_PEER_RECEIVE_ABORTED (0x%lX)",
            ErrorCode);
        (void)QuicStreamIndicateEvent(Stream, &Event);
    ...
    )
{
    ...
    if (Frame->Length == 0) {
        ...
        if (QuicRecvBufferGetTotalLength(&Stream->RecvBuffer) == Stream->MaxAllowedRecvOffset) {
            QuicTraceLogStreamVerbose("FlowControlExhausted: Flow control window exhausted!");
        }
	...
	if (Channel != NULL) 
	{
		if ((Channel->Mode == CLIENT) || (Channel->ConnState == QUIC_ACCEPT))
		{
			QUIC_SOCKFD* Context = Channel->Context;
    		Recv_Buffer* Buffer = (Recv_Buffer*)QuicAlloc(sizeof(Recv_Buffer));    
			if (!Buffer)         
			{           
     			Status = QUIC_STATUS_INVALID_STATE;
            	goto Error;
        	...
        	Buffer->Finish = Frame->Fin;
        	Buffer->Length = Frame->Length;
        	Buffer->Data = Frame->Data;
        	Buffer->FreeAddr = NULL;

        	QuicDispatchLockAcquire(&Channel->RecvList.Lock);
        	QuicListInsertTail(&Channel->RecvList.Data, &Buffer->Node);
...
Error:
    ...
    if (Status == QUIC_STATUS_INVALID_PARAMETER) {
        QuicTraceLogStreamWarning(
            "ReceiveTooBig: Tried to write beyond end of buffer!");
        QuicConnTransportError(Stream->Connection, QUIC_ERROR_FINAL_SIZE_ERROR);
    } else if (Status == QUIC_STATUS_BUFFER_TOO_SMALL) {
        QuicTraceLogStreamWarning(
            "ReceiveBeyondFlowControl: Tried to write beyond flow control limit!");
        QuicConnTransportError(Stream->Connection, QUIC_ERROR_FLOW_CONTROL_ERROR);
    }
    ...
    )
{
    ...
    case QUIC_FRAME_STREAM_DATA_BLOCKED: {
        ...
        QuicTraceLogStreamVerbose(
            "RemoteBlocked: Remote FC blocked (%lu)",
    ...
    )
{
    ...
    if (Stream->RecvWindowBytesDelivered >= RecvBufferDrainThreshold) {
        ...
        if (Stream->RecvBuffer.VirtualBufferLength <
            ...
            if (QuicTimeDiff32(Stream->RecvWindowLastUpdate, TimeNow) <= TimeThreshold) {
                ...
                QuicTraceLogStreamVerbose(
                    "IncreaseRxBuffer: Increasing max RX buffer size to %u (MinRtt=%u; TimeNow=%u; LastUpdate=%u)",
    ...
    QuicTraceLogStreamVerbose("UpdateFlowControl: Updating flow control window");
    ...
    )
{
    ...
    if (!Stream->Flags.ReceiveEnabled) {
        QuicTraceLogStreamVerbose("IgnoreRecvFlush: Ignoring recv flush (recv disabled)");
        return;
    }
    ...
    while (FlushRecv) {
        QUIC_DBG_ASSERT(!Stream->Flags.SentStopSending);
        ...
        QuicTraceLogStreamVerbose(
            "IndicateReceive: Indicating QUIC_STREAM_EVENT_RECEIVE [%lu bytes, %u buffers, 0x%x flags]",
    ...
    )
{
    ...
    Stream->Flags.ReceiveCallPending = FALSE;
    QuicTraceLogStreamVerbose(
        "ReceiveComplete: Recv complete (%lu bytes)",
        BufferLength);
    ...
    if (Stream->RecvBuffer.BaseOffset == Stream->RecvMaxLength) {
        ...
        QuicTraceLogStreamVerbose(
            "IndicatePeerSendShutdown: Indicating QUIC_STREAM_EVENT_PEER_SEND_SHUTDOWN");
        (void)QuicStreamIndicateEvent(Stream, &Event);
