    ...
    )
{
    if (QuicTraceLogVerboseEnabled()) {
        ...
        QuicTraceLogConnVerbose(
            "CryptoDump£ºQS:%u MAX:%u UNA:%u NXT:%u RECOV:%u-%u",
        ...
        while ((Sack = QuicRangeGetSafe(&Crypto->SparseAckRanges, i++)) != NULL) {
            QuicTraceLogConnVerbose(
                "CryptoDumpUnacked£º  unACKed: [%lu, %lu]",
                UnAcked,
                Sack->Low);
        ...
        if (UnAcked < (uint64_t)Crypto->MaxSentLength) {
            QuicTraceLogConnVerbose(
                "CryptoDumpUnacked2£º  unACKed: [%lu, %u]",
                UnAcked,
                Crypto->MaxSentLength);
        }
...
{
    QUIC_STATUS Status;
    QUIC_TLS_CONFIG TlsConfig;
	QuicZeroMemory(&TlsConfig, sizeof(QUIC_TLS_CONFIG));
    QUIC_CONNECTION* Connection = QuicCryptoGetConnection(Crypto);
    ...
    )
{
    ...
    QuicTraceLogConnInfo(
        "DiscardKeyType: Discarding key type = %hhu",
        KeyType);
    ...
    )
{
    ...
    if (BufferLength < *Offset + HeaderLength + 4) {
        QuicTraceLogConnVerbose(
            "NoMoreRoomForCrypto: No room for CRYPTO frame");
        return FALSE;
    }
    ...
    *FramePayloadBytes = (uint16_t)Frame.Length;
    ...
    QuicTraceLogConnVerbose(
        "AddCryptoFrame: Sending %hu crypto bytes, offset=%u",
    ...
    )
{
    ...
    if (UpdatedRecoveryWindow) {
        ...
        QuicTraceLogConnVerbose(
            "RecoverCrypto: Recovering crypto from %lu up to %lu",
            Start,
            End);
    ...
    )
{
    ...
    QuicTraceLogConnVerbose(
        "AckCrypto: Received ack for %u crypto bytes, offset=%u",
        Length,
        Offset);
    ...
    *DataReady = FALSE;
    ...
    if (Frame->Length == 0) {
        ...
        QuicTraceLogConnWarning(
            "IgnoreCryptoFrame: Ignoring received crypto after cleanup");
    ...
    QuicTraceLogConnVerbose(
        "RecvCrypto: Received %hu crypto bytes, offset=%lu Ready=%hhu",
        (uint16_t)Frame->Length,
        Frame->Offset,
        *DataReady);
    ...
    )
{
    ...
    if (ResultFlags & QUIC_TLS_RESULT_EARLY_DATA_ACCEPT) {
        QuicTraceLogConnInfo("ZeroRttAccepted: 0-RTT accepted");
    ...
    if (ResultFlags & QUIC_TLS_RESULT_EARLY_DATA_REJECT) {
        QuicTraceLogConnInfo("ZeroRttRejected: 0-RTT rejected");
        QUIC_TEL_ASSERT(Crypto->TlsState.EarlyDataState != QUIC_TLS_EARLY_DATA_ACCEPTED);
        if (!QuicConnIsServer(Connection)) {
    ...
    if (ResultFlags & QUIC_TLS_RESULT_WRITE_KEY_UPDATED) {
        QuicTraceLogVerbose(
            "ConnWriteKeyUpdated: [conn][%p] Write Key Updated, %hhu.",
            Connection,
    ...
    if (ResultFlags & QUIC_TLS_RESULT_READ_KEY_UPDATED) {
        ...
        QuicTraceLogVerbose(
            "ConnReadKeyUpdated: [conn][%p] Read Key Updated, %hhu.",
            Connection,
    ...
    if (ResultFlags & QUIC_TLS_RESULT_COMPLETE) {
        ...
        QuicTraceLogVerbose("ConnHandshakeComplete: [conn][%p] Handshake complete", Connection);
        ...
        if (QuicConnIsServer(Connection)) {
            ...
            //
            QuicTraceLogConnInfo("HandshakeConfirmedServer: Handshake confirmed (server)");
        ...
        QuicTraceLogConnVerbose(
            "IndicateConnected: Indicating QUIC_CONNECTION_EVENT_CONNECTED (Resume=%hhu)",
    ...
    )
{
    ...
    if (RecvBufferConsumed != 0) {
        ...
        QuicTraceLogConnVerbose("DrainCrypto: Draining %u crypto bytes", RecvBufferConsumed);
    ...
    )
{
    ...
    if (IsClientInitial) {
        ...
        if (Buffer.Length == 0) {
            QuicTraceLogConnVerbose("CryptoNotReady: No complete TLS messages to process");
            goto Error;
        }
...
Error:
    ...
    if (QUIC_FAILED(Status)) {
        ...
        *NewReadKey = NULL;
        ...
        QuicTraceLogVerbose(
            "ConnNewPacketKeys: [conn][%p] New packet keys created successfully.",
            Connection);
    }
    ...
    )
{
    ...
    *New = NULL;
    ...
    QuicTraceLogVerbose(
        "ConnKeyPhaseChange: [conn][%p] Key phase change (locally initiated=%hhu).",
        Connection,
        LocalUpdate);
