    ...
    )
{
    ...
    while (LossDetection->SentPackets != NULL) {
        ...
        if (Packet->Flags.IsAckEliciting) {
            QuicTraceLogVerbose(
                "PacketTxDiscarded: [%c][TX][%lu] Thrown away on shutdown",
    ...
    while (LossDetection->LostPackets != NULL) {
        ...
        QuicTraceLogVerbose(
            "PacketTxLostDiscarded: [%c][TX][%lu] Thrown away on shutdown (lost packet)",
            PtkConnPre(Connection),
            Packet->PacketNumber);
    ...
    )
{
    ...
    if (Connection->State.ClosedLocally || Connection->State.ClosedRemotely) {
        ...
        //
        QuicTraceLogVerbose(
            "ConnLossDetectionTimerCancel: [conn][%p] Cancelling loss detection timer.",
            Connection);
        QuicConnTimerCancel(Connection, QUIC_CONN_TIMER_LOSS_DETECTION);
        return;
    }
    ...
    if (Delay == 0 && ExecuteImmediatelyIfNecessary) {
    	...
    	QuicTraceLogVerbose(
        	"ConnLossDetectionTimerSet: [conn][%p] Setting loss detection %hhu timer for %u ms. (ProbeCount=%hu)",
        	Connection,
        	TimeoutType,
        	Delay,
    ...
    )
{
    ...
    if (SentPacket == NULL) {
		QuicTraceLogError("Allocation of '%s' failed. (%lu bytes)",
    ...
    )
{
    ...
    if (QuicConnIsClient(Connection) &&
        ...
        QuicTraceLogConnInfo(
            "HandshakeConfirmedAck: Handshake confirmed (ack)");
...
#pragma prefast(suppress:6001, "TODO - Why does compiler think: Using uninitialized memory '*DestCid'")
    ...
    if (Path != NULL) {
        ...
        if (Packet->Flags.IsMtuProbe) {
            QUIC_DBG_ASSERT(Path->IsMinMtuValidated);
        ...
        case QUIC_FRAME_PATH_CHALLENGE: {
            ...
            if (Path != NULL && !Path->IsPeerValidated) {
                ...
                if (QuicTimeDiff32(Path->PathValidationStartTime, TimeNow) > ValidationTimeout) {
                    QuicTraceLogConnInfo(
                        "PathValidationTimeout: Path[%hhu] validation timed out",
                        Path->ID);
    ...
    )
{
    ...
    if (LossDetection->LostPackets != NULL) {
        ...
        while ((Packet = LossDetection->LostPackets) != NULL &&
            ...
            QuicTraceLogVerbose(
                "PacketTxForget: [%c][TX][%lu] Forgetting",
                PtkConnPre(Connection),
                Packet->PacketNumber);
        ...
        while (Packet != NULL) {
            ...
            if (Packet->PacketNumber + QUIC_PACKET_REORDER_THRESHOLD < LossDetection->LargestAck) {
                if (!NonretransmittableHandshakePacket) {
                    QuicTraceLogVerbose(
                        "PacketTxLostFack: [%c][TX][%lu] Lost: FACK %lu packets",
                        PtkConnPre(Connection),
                        Packet->PacketNumber,
                        LossDetection->LargestAck - Packet->PacketNumber);
                    QuicTraceLogVerbose(
                        "ConnPacketLost: [conn][%p][TX][%lu] %hhu Lost: %hhu",
                        Connection,
                        Packet->PacketNumber,
    ...
    )
{
    ...
    while (Packet != NULL) {
        ...
        if (Packet->Flags.KeyType == KeyType) {
            ...
            QuicTraceLogVerbose(
                "PacketTxAckedImplicit: [%c][TX][%lu] ACKed (implicit)",
                PtkConnPre(Connection),
                Packet->PacketNumber);
            QuicTraceLogVerbose(
                "ConnPacketACKed: [conn][%p][TX][%lu] %hhu ACKed",
                Connection,
                Packet->PacketNumber,
                QuicPacketTraceType(Packet));
    ...
    )
{
    ...
    while (Packet != NULL) {
        ...
        if (Packet->Flags.KeyType == QUIC_PACKET_KEY_0_RTT) {
            ...
            QuicTraceLogVerbose(
                "PacketTx0RttRejected: [%c][TX][%lu] Rejected",
                PtkConnPre(Connection),
                Packet->PacketNumber);
    ...
    while ((AckBlock = QuicRangeGetSafe(AckBlocks, i++)) != NULL) {
        ...
        if (*LostPacketsStart != NULL) {
            ...
            while (*End && (*End)->PacketNumber <= QuicRangeGetHigh(AckBlock)) {
                QuicTraceLogVerbose(
                    "PacketTxSpuriousLoss: [%c][TX][%lu] Spurious loss detected",
                    PtkConnPre(Connection),
                    (*End)->PacketNumber);
    ...
    )
{
    ...
    while (Packet != NULL) {
        if (Packet->Flags.IsAckEliciting) {
            QuicTraceLogVerbose(
                "PacketTxProbeRetransmit: [%c][TX][%lu] Probe Retransmit",
                PtkConnPre(Connection),
                Packet->PacketNumber);
            QuicTraceLogVerbose(
                "ConnPacketLost: [conn][%p][TX][%lu] %hhu Lost: %hhu",
