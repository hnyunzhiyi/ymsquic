    ...
    )
{
    ...
    QuicTraceLogInfo(
        "PerfDriverStarted: [perf] Started");
    ...
    )
{
    ...
    QuicTraceLogInfo(
        "PerfDriverStopped: [perf] Stopped");
    ...
    )
{
    ...
    QuicTraceLogVerbose(
        "PerfControlInitialized: [perf] Control interface initialized with %.*S", QuicPerfCtlDeviceName.Length, QuicPerfCtlDeviceName.Buffer);
    ...
    QuicTraceLogVerbose(
        "PerfControlInitialized: [perf] Control interface initialized");
...
{
    QuicTraceLogVerbose(
        "PerfControlUninitializing: [perf] Control interface uninitializing");
    ...
    QuicTraceLogVerbose(
        "PerfControlUninitialized: [perf] Control interface uninitialized");
}
    ...
    )
{
    ...
    do {
        ...
        QuicTraceLogInfo(
            "PerfControlClientCreated: [perf] Client %p created",
            Client);
    ...
    )
{
    ...
    if (Client != nullptr) {
        ...
        QuicTraceLogInfo(
            "PerfControlClientCleaningUp: [perf] Client %p cleaning up",
            Client);
    ...
    )
{
    ...
    QuicTraceLogWarning(
        "PerfControlClientCanceledRequest: [perf] Client %p canceled request %p",
        Client,
        Request);
...
QUIC_THREAD_CALLBACK(PerformanceWaitForStopThreadCb, Context)
{
    ...
    if (Client->Canceled) {
        QuicTraceLogInfo(
            "PerformanceStopCancelled: [perf] Performance Stop Cancelled");
    ...
    QuicTraceLogInfo(
        "PrintBufferReturn: [perf] Print Buffer %d %s\n",
        BufferCurrent,
        LocalBuffer);
    ...
    )
{
    ...
    QuicTraceLogInfo(
        "PerfControlClientIoctl: [perf] Client %p executing write IOCTL %u",
        Client,
        FunctionCode);
...
Error:
    QuicTraceLogInfo(
        "PerfControlClientIoctlComplete: [perf] Client %p completing request, 0x%x",
        Client,
        Status);
