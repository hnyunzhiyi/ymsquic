...
struct SpinQuicGlobals {
    ...
    SpinQuicGlobals() { QuicZeroMemory(Buffers, sizeof(Buffers)); }
    ~SpinQuicGlobals() {
...
void Spin(Gbs& Gb, LockableVector<HQUIC>& Connections, std::vector<HQUIC>* Listeners = nullptr)
{
    ...
    while (++OpCount != Settings.MaxOperationCount &&
        QuicTimeDiff64(Gb.StartTimeMs, QuicTimeMs64()) < Settings.RunTimeMs) {
...
BOOLEAN QUIC_API DatapathHookSendCallback(QUIC_ADDR* /* RemoteAddress */, QUIC_ADDR* /* LocalAddress */, struct QUIC_SEND_DATA* /* SendData */)
{
...
QUIC_THREAD_CALLBACK(RunThread, Context)
{
...
main(int argc, char **argv)
{
    ...
    if (!TryGetValue(argc, argv, "seed", &RngSeed)) {
      	QuicRandom(sizeof(RngSeed), &RngSeed);
    }
    ...
    QuicCopyMemory(&MsQuic, TempMsQuic, sizeof(MsQuic));
    ...
    for (uint32_t i = 0; i < RepeatCount; i++) {
        ...
        QUIC_THREAD Threads[4];
        const uint32_t Count = (uint32_t)(rand() % (ARRAYSIZE(Threads) - 1) + 1);

        for (uint32_t j = 0; j < Count; ++j) {
            ASSERT_ON_FAILURE(QuicThreadCreate(&Config, &Threads[j]));
        }

        for (uint32_t j = 0; j < Count; ++j) {
            QuicThreadWait(&Threads[j]);
            QuicThreadDelete(&Threads[j]);
        }
    }
